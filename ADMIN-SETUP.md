...
## 대시보드에서 보이는 것
- **접속 통계**: 총 방문 · 순 세션 · 평균 체류시간 · 오늘 방문 · 국가별 · 일자별 · 기기 · 언어 · 인기 페이지 · 유입 경로
- **프로젝트/CTA 관심도**: 어떤 프로젝트 카드를 많이 열었는지, 이메일·GitHub·LinkedIn·Live 링크 클릭 수
- **연락 폼 인박스**: 방문자가 남긴 메시지(안읽음 표시 · 읽음 처리)
- **라이트 CMS**: `content` 테이블에 프로젝트/수상 데이터를 JSON으로 추가·수정
